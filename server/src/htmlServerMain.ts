        ...
        if (!promise) {
            ...
            let configRequestParam: ConfigurationParams = {
                items: [
                    ...
                    { scopeUri, section: "less" },
                ],
            };
            promise = connection.sendRequest(ConfigurationRequest.type, configRequestParam).then((s) => ({ css: s[0], html: s[1], javascript: s[2], less: s[3] }));
...
connection.onInitialize((params: InitializeParams): InitializeResult => {
    ...
    languageModes = getLanguageModes(initializationOptions ? initializationOptions.embeddedLanguages : { css: true, javascript: true, less: true });
    documents.onDidClose((e) => {
    ...
    return runSafe(
        async () => {
            ...
            if (emmetCompletionList && emmetCompletionList.items) {
                ...
                return {
                    isIncomplete: true,
                    items: [...emmetCompletionList.items, ...result.items],
                };
            }
            return result;
        },
...
connection.onDocumentFormatting(async (param, token) => {
    return runSafe(
        async () => {
            ...
            if (!document || document.languageId !== "blade") {
                return null;
            }

            const htmlLanguageService = getHTMLLanguageService();
            const documentRange = Range.create(document.positionAt(0), document.positionAt(document.getText().length - 1));

            // html格式化与embbedded格式化存在冲突需要先使用html进行格式化后再进行embbedded格式化
            let foramtedText = htmlLanguageService.format(document, documentRange, param.options)[0].newText;

            const formattedDocument = TextDocument.create(document.uri, document.languageId, document.version + Math.round(Math.random()) * 9999, foramtedText);
            const formatedDocumentRange = Range.create(formattedDocument.positionAt(0), formattedDocument.positionAt(foramtedText.length));

            const documentRegions = embeddedSupport.getDocumentRegions(htmlLanguageService, formattedDocument);
            const regions = documentRegions.getLanguageRanges(formatedDocumentRange);

            const changes: TextEdit[] = [];
            regions
                .filter((r) => r.languageId !== "html")
                .forEach((region) => {
                    const mode = languageModes.getMode(region.languageId);
                    if (mode.format) {
                        const embbeddedRange = Range.create(region.start, region.end);
                        changes.push(...mode.format(formattedDocument, embbeddedRange, param.options));
                    }
                });

            foramtedText = TextDocument.applyEdits(formattedDocument, changes);

            const bladeFormater = new BladeFormatter(param.options);
            foramtedText = bladeFormater.format(foramtedText);

            connection.workspace.applyEdit({
                changes: {
                    [param.textDocument.uri]: [TextEdit.replace(documentRange, foramtedText)],
                },
            });
        ...
        [],
        `Error while formatting range for ${param.textDocument.uri}`
    );
});
